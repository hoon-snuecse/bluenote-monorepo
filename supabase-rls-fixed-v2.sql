-- Supabase 대시보드에서 실행할 최종 스크립트 (v2)
...
DROP POLICY IF EXISTS "Users can insert options for their questions" ON public.question_options;
DROP POLICY IF EXISTS "Users can update options for their questions" ON public.question_options;
DROP POLICY IF EXISTS "Users can delete options for their questions" ON public.question_options;
...
-- 기존 샘플 데이터 삭제 (중복 방지)
DELETE FROM public.quizzes WHERE is_sample = true;
