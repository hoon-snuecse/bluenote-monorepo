...
export default function SignInClient() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Supabase OAuth flow 사용 - PKCE flow 명시
      const { data, error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          queryParams: {
            access_type: 'offline',
            // prompt 제거 - 기본값 사용 (필요시만 동의 화면 표시)
          },
      ...
      if (error) {
        ...
        console.log('[SignInClient] OAuth initiated successfully');
      }
    } catch (err) {
      console.error('[SignInClient] Unexpected error:', err);
    }
  };
